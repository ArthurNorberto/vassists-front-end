...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader', '$rootScope',
      function ($document, $injector, $uibModal, $timeout, ImageLoader, $rootScope) {
    // optional dependency
    ...
    Lightbox.openModal = function (newImages, newIndex, modalParams) {
      ...
      Lightbox.modalInstance.opened.then(function(){
        Lightbox.isModalOpen = true;
      });
    ...
    Lightbox.prevImage = function () {
      ...
      $rootScope.$broadcast("lightbox-prevImage", { index : (Lightbox.index - 1 + Lightbox.images.length) %
        Lightbox.images.length});
    };
    ...
    Lightbox.nextImage = function () {
      Lightbox.setImage((Lightbox.index + 1) % Lightbox.images.length);
      $rootScope.$broadcast("lightbox-nextImage", { index : (Lightbox.index + 1) % Lightbox.images.length});
    };
